...
class SoftActorCritic(nn.Module):
    def __init__(
        ...
        if self.auto_tune_temperature:
            ...
            init_temp = max(self.temperature, 1e-8)
            self.log_alpha = torch.tensor(
                np.log(init_temp), dtype = torch.float32, device = ptu.device, requires_grad = True
            )
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_entropy_bonus and self.backup_entropy:
                ...
                next_action_entropy = next_action_distribution.log_prob(next_action)
                # Hint: next_qs = ...
            ...
            reward_b = reward[None]
            not_done_b = (1.0 - done.float())[None]
            target_values = reward_b + self.discount * not_done_b * next_qs
            # ENDTODO
    ...
    def entropy(self, action_distribution: torch.distributions.Distribution):
        ...
        if getattr(action_distribution, "has_rsample", False):
            a = action_distribution.rsample()
        else:
    ...
    def actor_loss_reparametrize(self, obs: torch.Tensor):
        ...
        if self.num_critic_networks > 1 and self.target_critic_backup_type == 'min':
            q_for_policy = q_values.min(dim = 0).values
        else:
            q_for_policy = q_values.mean(dim = 0)
        loss = -q_for_policy.mean()
        # ENDTODO
