...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        if self.critic is not None:
            ...
            for _ in range(self.baseline_gradient_steps):
                ...
                if isinstance(critic_info, dict):
                    critic_info_agg.update(critic_info)
            info.update(critic_info_agg)
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t, r in enumerate(rewards):
            total_discounted_return += (self.gamma ** t) * r
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for t in reversed(range(len(rewards))):
            if t == len(rewards) - 1:
                discounted_reward_to_go[t] = rewards[t]
            else:
                discounted_reward_to_go[t] = rewards[t] + self.gamma * discounted_reward_to_go[t + 1]
        return discounted_reward_to_go.tolist()
    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        if not self.use_reward_to_go:
            ...
            q_values = [np.array(self._discounted_return(reward), dtype = np.float32) for reward in rewards]
        else:
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    advantages[i] = delta + self.gamma * self.gae_lambda * advantages[i + 1] * nonterminal
