...
def compute_group_advantages(rewards: torch.Tensor, group_size: int, eps: float = 1e-6) -> torch.Tensor:
    ...
    n_rewards = rewards.numel()
    if n_rewards % group_size != 0:
        raise ValueError(
            f"rewards.numel()={rewards.numel()} must be divisible by group_size={group_size}"
        )
    num_groups = n_rewards // group_size
    ...
    adv = torch.where(std > eps, adv, torch.zeros_like(adv))
